...
for ($i = 0; $i < $numberofusers; $i++) {
    ...
    $submissions = block_completion_progress_submissions($course->id, $users[$i]->id);
...
for ($i = $startuser; $i < $enduser; $i++) {
    ...
    if (!empty($useractivities)) {
        // Submission data with attempt ids 
    ...
    $rows[$i] = array(
        ...
        'progress' => $progress,
        'submissions'=> $users[$i]->submissions
    );
}
...
if ($paged) {
    ...
    echo $OUTPUT->container(html_writer::link($perpageurl, get_string('showall', '', $numberofusers)), array('target' => '_blank'), 'showall');
} else if ($numberofusers > DEFAULT_PAGE_SIZE) {
    $perpageurl->param('perpage', DEFAULT_PAGE_SIZE);
    echo $OUTPUT->container(html_writer::link($perpageurl, get_string('showperpage', '', DEFAULT_PAGE_SIZE)), array('target' => '_blank'), 'showall');
}
