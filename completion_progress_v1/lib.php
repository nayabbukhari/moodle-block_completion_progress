...
function block_completion_progress_bar($attempt, $activities, $completions, $config, $userid, $courseid, $instance, $simple = false) {
    ...
    if ($showpercentage == 1 && !$simple) {
        ...
        if ($progress_student == 100 && $progress_teacher < 95 ) {
            $percentagecontent = 'Student : '.$progress_student.'%, Please wait for result </br>'.
...
/**
 * Email to admin 
 * if student completed 100% within last 2 days
 ...
 * 
*/
function email_to_admin($context, $courseid, $studentid) {
    ...
    $subject = 'Assessment Submission Notification';
        ...
        foreach ($managers as $touser){
            $message = 'Hi <b>'.$touser->firstname. ' '.$touser->lastname.'</b>
                        ...
                        assessment submission for <b>'.$courselink.'</b>
                        </br>
                        is ready to be allocated for marking in Creatine 2.
                        </br>     
                        Regards
                        </br>
                        <b>Australian College</b>';
                  
            email_to_user($touser, $touser, $subject, $message, '', '', '', false);
        }
    return true;
}
...
function progress_col($progressvalue, $progressvalue_trainer, $user, $context, $courseid, $instanceid ) {
    ...
    if ($progressvalue == 100 && is_siteadmin() == 1  && $progressvalue_trainer < 50) {
        
        // email to all student with 100% completed in last ? days.
        $userdate = $user->lastonlinetime;
        $notifyperiod = $user->lastonlinetime;
        $userdate = strtotime("-1 Days");
        
        $text= 'Assign Trainer';

        //send email to teacher if student completed within last 2 days.
        if ($notifyperiod > $userdate ) {
            $emailto = 'admin';
            email_to_admin ($context, $courseid, $user->id);
        } else {
            $emailto = 'teacher';
        }
        ...
        $link = new moodle_url($CFG->wwwroot.'/blocks/completion_progress/email.php', 
                    array('instanceid' => $instanceid, 'courseid' => $courseid, 'studentid' => $user->id, 
                            'sesskey' => sesskey(), 'email' => $emailto));
            
        $progress .= $OUTPUT->action_link($link, $text, null, ['class' => 'btn btn-sm btn-primary', 'target' => '_blank']);
    ...
    if ($progressvalue_trainer > 95 && $trainerid != false){

        // Display "Release result" button to course trainer, if trainer progress is equal 100% 
        $text= 'Release result';
        ...
        $progress .= $OUTPUT->action_link($link, $text, null, ['class' => 'btn btn-sm btn-success', 'target' => '_blank']);
